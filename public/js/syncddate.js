document.addEventListener('DOMContentLoaded', function () {
    ...
    const hiddenPriceInput = document.getElementById('hidden-price'); // Campo oculto para sincronizar precio
    ...
    function calculatePrice() {
        ...
        axios.get(`/boats/${boatId}/daily-price`, {
            ...
            hiddenPriceInput.value = totalPrice; // Sincroniza el precio con el campo oculto
            priceSummary.style.display = 'block';
        }).catch(error => {
    ...
    // Sincronizar el precio antes de enviar el formulario
    reservationForm.addEventListener('submit', function (event) {
        const totalPrice = totalPriceElement.textContent.replace('€', '').trim();
        if (!totalPrice || totalPrice === 'Error') {
            event.preventDefault(); // Detener el envío si no hay precio válido
            alert('Por favor selecciona fechas válidas para calcular el precio.');
            return;
        }
        hiddenPriceInput.value = totalPrice; // Actualiza el campo oculto con el precio visible
    });
