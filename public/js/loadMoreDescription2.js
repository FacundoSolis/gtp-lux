document.addEventListener('DOMContentLoaded', function () {
  const descriptionButton = document.getElementById('loadMoreDescription2Button');
  const modalContainer = document.querySelector('.modal-description-container');

  // Contenido de la descripción para el modal
  ...
  // Función que carga la descripción y muestra el modal
  const loadDescription = () => {
      modalContainer.innerHTML = descriptionContent;
      
      // Muestra el modal usando Bootstrap
      const descriptionModal = new bootstrap.Modal(document.getElementById('descriptionModal2'));
      descriptionModal.show();

      // Ocultar el botón después de cargar la descripción
      descriptionButton.style.display = 'none';
  };

  // Asigna el evento click al botón para cargar la descripción y abrir el modal
  descriptionButton.addEventListener('click', loadDescription);

  // Vuelve a mostrar el botón cuando se cierra el modal
  const descriptionModalElement = document.getElementById('descriptionModal2');
  descriptionModalElement.addEventListener('hidden.bs.modal', function () {
      descriptionButton.style.display = 'block'; // Muestra el botón nuevamente
  });
});
